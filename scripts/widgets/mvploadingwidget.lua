...
local original_card_anims = GorgeEnv.MVPEmotes()
...
function MVPLoadingWidget:PopulateData()
	...
	local card_anims = deepcopy(original_card_anims)
	...
	if mvp_cards ~= nil and #mvp_cards > 0 then
		...
		for i, data in ipairs(mvp_cards) do
			...
			if #original_card_anims > 1 then
				table.remove(card_anims, random_anim)
			end
			
			if #card_anims == 0 then
				card_anims = deepcopy(original_card_anims)
			end
