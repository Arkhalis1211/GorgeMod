...
local function ToggleUpdate(force)
    if not GetGorgeGameModeProperty("darkness") and _maxbirds > 0 then
...
function self:GetSpawnPoint(pt)
    ...
    local function TestSpawnPoint(offset)
        ...
        return _map:IsPassableAtPoint(spawnpoint:Get()) and 
               ...
               #(TheSim:FindEntities(spawnpoint.x, 0, spawnpoint.z, 4, { "birdblocker" })) == 0 and
			   tile ~= 1
    end
...
function self:SpawnBird(spawnpoint, ignorebait)
    ...
    if bird.components.eater and not ignorebait then
		-- Fox: Why do they check all prefabs??
        local bait = TheSim:FindEntities(spawnpoint.x, 0, spawnpoint.z, 15, {"bait"})
